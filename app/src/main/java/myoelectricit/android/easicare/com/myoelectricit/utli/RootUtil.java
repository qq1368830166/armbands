...
//                gestureData="解锁";
    ...
    private static void abc(List<Byte> dataList) {
        ...
        if (recvData.length == 32) {
            Log.e("aaaaa每三秒检测是否同步", "rec[18]==" + recvData[18]);
            Log.e("aaaaa每三秒检测是否正在预热中", "rec[19]==" + recvData[19]);
            if (recvData[18] == 0x0a) {
                synNum++;
                Log.e("--BleBack--", "====synNum====" + synNum);
                synchro = "未同步";
                Log.e("--BleBackground", "未同步");
                if (synNum == 3) {
                    synNum = 0;
                }
            } else if (recvData[18] == 0x0c) {
                Log.e("--BleBackground", "rec[18]接收到0c,不作处理");
            } else {
                synchro = "同步";
                Log.e("--BleBackground", "同步了~~~");
                synNum = 0;
            }
            Log.e("当前指示   ", synchro+"啦啦啦?");
